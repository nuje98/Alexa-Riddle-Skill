...
const RiddleIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        const speechOutput = speakOutput + data['QUESTIONS'][questionIndex];
        const dataToSave = {
            "RIGHT_ANSWER" : data["ANSWERS"][questionIndex]
        }
        handlerInput.attributesManager.setSessionAttributes(dataToSave);
        const reprompt = data['QUESTIONS'][questionIndex] +" "+ data["ANSWER_MESSAGE"];
        return handlerInput.responseBuilder
            .speak(speechOutput)
            .reprompt(reprompt)
            .getResponse();
...
const AnswerIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        let speakOutput = "Wrong Answer. You only have x chances remaining."; // default wrong answer
